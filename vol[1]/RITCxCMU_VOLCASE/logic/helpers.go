...
func ShouldTrade(currentIV, low, high float64, forecastType string) bool {
	switch forecastType {
	case "increase":
		return currentIV < high-1.0
	case "decrease":
		return currentIV > low+1.0
	case "range", "fixed":
		return currentIV < low || currentIV > high
	default:
		return false
	}
}

func PartialClose(pos *OptionPosition, currentIV float64) int {
	if pos.Action == "BUY" && currentIV >= pos.EntryIV+(pos.TargetIV-pos.EntryIV)/2 {
		closed := pos.Quantity / 2
		pos.Quantity -= closed
		return closed
	} else if pos.Action == "SELL" && currentIV <= pos.EntryIV-(pos.EntryIV-pos.TargetIV)/2 {
		closed := pos.Quantity / 2
		pos.Quantity -= closed
		return closed
	}
...
func AdjustTargetIV(currentIV, originalTarget float64, forecastType string) float64 {
	switch forecastType {
	case "increase":
		return math.Max(currentIV+2, originalTarget)
	case "decrease":
		return math.Min(currentIV-2, originalTarget)
	case "range", "fixed":
		return originalTarget // keep same
	default:
		return originalTarget
	}
}

func ShouldHedge(totalDelta float64, band float64) bool {
	return math.Abs(totalDelta) > band/2 // tighter hedging band
}

func NearestStrike(S float64, allowed []int) int {
	best, diff := allowed[0], math.MaxFloat64
	for _, k := range allowed {
		if d := math.Abs(float64(k) - S); d < diff {
			diff, best = d, k
		}
	}
	return best
...
// CalcOrderQty calculates dynamic order size based on current IV and target IV
func CalcOrderQty(currentIV, targetIV float64, maxContracts int) int {
	diff := math.Abs(targetIV - currentIV)
	scale := 1.0 + diff/10.0 // increase size for bigger gaps
	qty := int(float64(maxContracts) * scale)
	if qty > maxContracts*2 {
		qty = maxContracts * 2
	}
	if qty < 1 {
		qty = 1
