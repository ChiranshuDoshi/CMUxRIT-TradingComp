...
def accept_tender_with_partial_takeprofit(offer: dict, snapshot: dict):
    """
    Executes tender with partial take profit (80%) and trailing stop (10 sec).
    """
    tender_id = offer.get("tender_id")
    tender_price_usd = float(offer.get("price", 0.0))
    tender_size = int(offer.get("size", 1))
    ...
    # Determine quantity for partial take profit
    qty_takeprofit = int(tender_size * 0.8)
    qty_remaining = tender_size - qty_takeprofit

    # Target prices
    target_profit = tender_cad + 0.15
    trailing_stop_time = 10  # seconds
    exit_done = False
    start_time = time.time()

    while not exit_done and time.time() - start_time < trailing_stop_time:
        bull_bid, bull_ask = best_bid_ask(BULL)
        bear_bid, bear_ask = best_bid_ask(BEAR)
        usd_bid, usd_ask = best_bid_ask(USD)
        usd_mid_now = (usd_bid + usd_ask) / 2
        basket_mid_now = bull_ask + bear_ask
        ritc_bid, ritc_ask = best_bid_ask(RITC)
        ritc_cad = ritc_bid * usd_mid_now if action == "SELL" else ritc_ask * usd_mid_now

        # Profit calculation
        profit = basket_mid_now - tender_cad if action == "BUY" else tender_cad - basket_mid_now

        # Take partial profit
        if profit >= 0.15:
            print(f"[TENDER] TAKE PROFIT 80% id={tender_id} profit={profit:.4f}")
            if action == "BUY":
                exec_buy_basket_units(qty_takeprofit)
                place_market(RITC, "SELL", qty_takeprofit)
            else:
                exec_sell_basket_units(qty_takeprofit)
                place_market(RITC, "BUY", qty_takeprofit)
            exit_done = True
            break
    ...
    if qty_remaining > 0:
        print(f"[TENDER] TRAILING STOP 20% id={tender_id}")
        if action == "BUY":
            exec_buy_basket_units(qty_remaining)
            place_market(RITC, "SELL", qty_remaining)
        else:
            exec_sell_basket_units(qty_remaining)
            place_market(RITC, "BUY", qty_remaining)
...
def main_loop():
    ...
    while True:
        ...
        # 1️⃣ Check profit-taking positions
        close_profitable_positions(snapshot, profit_threshold=0.15)

        # Tendors first (time sensitive)
        tenders = get_active_tenders()
        for t in tenders:
            accept_tender_with_trailing_stop(t, snapshot)
