...
def predict_market_direction(snapshot_history):
    """
    Simple short-term predictor using order book imbalance and recent price changes.
    Returns +1 if market likely moves up, -1 if likely moves down, 0 if neutral.
    snapshot_history: list of last N snapshots (dicts)
    """
    if len(snapshot_history) < 3:
        return 0  # not enough data

    # Measure trend in basket and ETF
    basket_prices = [s['bull_ask'] + s['bear_ask'] for s in snapshot_history]
    ritc_prices = [s['ritc_ask_cad'] for s in snapshot_history]

    basket_trend = np.mean(np.diff(basket_prices))  # positive if basket rising
    ritc_trend = np.mean(np.diff(ritc_prices))      # positive if ETF rising

    # Market likely direction = basket vs ETF
    edge_trend = basket_trend - ritc_trend
    if edge_trend > 0.01:
        return +1  # market likely rises
    elif edge_trend < -0.01:
        return -1  # market likely falls
    else:
        return 0   # neutral

import time

def accept_tender_quick_profit(offer: dict, snapshot: dict, profit_target: float = 5000):
    """
    Accept tender only if we can make profit_target CAD within tender's life (8 seconds).
    Instantly unwind after hitting target.
    """
    tender_id = offer.get("tender_id")
    tender_price_usd = float(offer.get("price", 0.0))
    tender_size = int(offer.get("size", 1))
    ...
    if tender_cad < basket_mid - 0.1:
        ...
        print(f"[TENDER] SKIP tender id={tender_id}, not profitable enough")
        return False
    ...
    while time.time() - start_time < 8.0:
        ...
        usd_bid, usd_ask = best_bid_ask(USD)
        usd_mid = (usd_bid + usd_ask) / 2
        basket_now = bull_bid + bear_bid if action == "SELL" else bull_ask + bear_ask

        # Estimate profit
        if action == "BUY":
            profit = (basket_now - tender_cad) * tender_size
        else:
            profit = (tender_cad - basket_now) * tender_size
        ...
        if profit >= profit_target:
            ...
            if not (r and r.ok):
                print(f"[TENDER] accept failed id={tender_id}")
                return False
            print(f"[TENDER] ACCEPTED id={tender_id} profit={profit:.2f} CAD, instant unwind")

            # Instant unwind
            if action == "BUY":
                exec_buy_basket_units(tender_size)
                place_market(RITC, "SELL", tender_size)
            else:
                exec_sell_basket_units(tender_size)
                place_market(RITC, "BUY", tender_size)
    ...
    if not accepted:
        print(f"[TENDER] SKIPPED id={tender_id}, did not reach profit target within 8s")
...
def main_loop():
    ...
    while True:
        ...
        for t in tenders:
            accept_tender_quick_profit(t, snapshot, profit_target=5000)
